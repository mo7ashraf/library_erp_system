    ...
    <div class="ledger-page">
        ...
        <div class="ledger-card">
            <div class="ledger-header">
                <div>
                    <h2 class="ledger-title">كشف حساب مورد</h2>
                    <div class="ledger-subtitle">
                        اختر المورد والفترة الزمنية لعرض تفاصيل الرصيد والحركات.
                    </div>
                </div>
                ...
                <form method="GET">
                    <div class="ledger-filter-grid">
                        <div class="ledger-field">
                            ...
                            <select name="supplier_id">
                                @foreach($suppliers as $id => $name)
                                    <option value="{{ $id }}" @selected((int) $supplierId === (int) $id)>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="ledger-actions">
                            ...
                            <a
                                href="{{ route('admin.prints.supplier-ledger', [
        ...
        @if(! $supplierId)
            <div class="ledger-card">
                <div class="ledger-card-body">
                    لا يوجد موردين لعرض كشف الحساب.
                </div>
            </div>
        @else
            <div class="ledger-card">
                <div class="ledger-card-body">
                    <div class="ledger-info-grid">
                        <div>
                            <div class="ledger-label">بيانات المورد</div>
                            <div class="ledger-party-name">{{ $ledger['party_name'] ?? '-' }}</div>
                            <div class="ledger-party-code">كود المورد: {{ $ledger['party_code'] ?? '-' }}</div>
                        </div>
                    ...
                    <table class="ledger-table">
                        ...
                        @forelse($visibleRows as $row)
                            ...
                            <tr>
                                ...
                                <td class="text-left ledger-debit">
                                    {{ $debit > 0 ? $money($debit) : '-' }}
                                </td>
                                <td class="text-left ledger-credit">
                                    {{ $credit > 0 ? $money($credit) : '-' }}
                                </td>
                                <td class="text-left {{ $balanceClass($balance) }}">
                                    {{ $row['balance_label'] }}
                                </td>
                            </tr>
                        @empty
